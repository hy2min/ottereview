...
const CustomSelect = ({
  ...
  const dropdownClasses = twMerge(
    ...
    'absolute z-50 top-full left-0 right-0 overflow-y-auto rounded-b-lg border-t-transparent'
  )

  return (
    <div ref={selectRef} className="relative w-full" {...props}>
      <div className={selectClasses} onClick={() => setIsOpen(!isOpen)}>
        <div className="flex justify-between items-center">
          <span className={selectedOption ? 'theme-text' : 'theme-text-muted'}>
            {selectedOption ? selectedOption.label : placeholder}
          </span>
          <span className="theme-text-secondary">{isOpen ? '▲' : '▼'}</span>
        </div>
      </div>

      {isOpen && (
        <div className={dropdownClasses} style={{ maxHeight: `${maxHeight * 4}px` }}>
          {options.map((option) => (
            <div
